 ...
 * 
 * This class encapsulates both a forward and inverted index which are used to store information
 * about the files the clone detection is being performed on.
 * 
 */
...
public class Index implements Iterable<BigInteger> {
	...
	/**
	 * Updates both the forward and inverted indexes by reading in the supplied filename
	 * with the supplied algorithm to compute the fingerprint.
	 * @param filename The filename to be indexed.
	 * @param algorithm The algorithm to be used to compute the fingerprint for each line.
	 ...
	 * For the forward index, a key is created for the reference and the fingerprint
	 * is stored as the value.
	 * For the inverted index, a key and postings list is created for the fingerprint
	 * if it doesn't already exist and then the reference is added to the postings list.
	 * @param fingerprint
	 * @param reference
	 */
	 ...
	 * @param fingerprint The fingerprint to be looked up in the inverted index.
	 * @return The postings list of references that correspond to that fingerprint.
	 */
	...
	/**
	 * Does a forward lookup on the index. For the supplied reference, the corresponding 
	 * fingerprint will be returned. This saves the line having to be stored and the
	 * fingerprint recomputed every time it is required.
	 * @param ref The reference to be looked up.
